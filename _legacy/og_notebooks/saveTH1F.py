...
xsec = {"signal": 1.0, "tt_semilep": 831.76 * 0.438, "tt_had": 831.76 * 0.457}
...
for sample in hists.keys():
    ...
    roothists[sample + "_nominal"] = TH1F(
        "mtt__" + sample, ";m_{t#bar{t}} (GeV);events", 50, 0, 3000
    )
    roothists[sample + "_nominal"].FillN(len(data), data, nominalweight)
    roothists[sample + "_puUp"] = TH1F(
        "mtt__" + sample + "__puUp", ";m_{t#bar{t}} (GeV);events", 50, 0, 3000
    )
    roothists[sample + "_puUp"].FillN(len(data), data, puweight_up)
    roothists[sample + "_puDn"] = TH1F(
        "mtt__" + sample + "__puDn", ";m_{t#bar{t}} (GeV);events", 50, 0, 3000
    )
