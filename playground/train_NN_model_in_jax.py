...
PHYSICS_PROCESSES = {
    ...
    "ttbar_had":
        "preproc_uproot/z-prime-ttbar-data/ttbar_had__nominal/file__0/part0.root",
    "ttbar_lep":
        "preproc_uproot/z-prime-ttbar-data/ttbar_lep__nominal/file__0/part0.root",
    "ttbar_semilep":
        "preproc_uproot/z-prime-ttbar-data/ttbar_semilep__nominal/file__0/part0.root",
...
def load_or_process_physics_data(output_dir: Path, reprocess: bool):
    ...
    for var in PHYSICS_VARIABLES:
        data["ttbar"][var] = np.concatenate(
            [
                data[p][var]
                for p in ["ttbar_had", "ttbar_lep", "ttbar_semilep"]
                if p in data
            ]
        )
...
def plot_model_scores(
    ...
    X_wjets = np.column_stack(
        [data["wjets"][v] * s for v, s in VARS_TO_USE.items()]
    )
    X_ttbar = np.column_stack(
        [data["ttbar"][v] * s for v, s in VARS_TO_USE.items()]
    )
    X_signal = np.column_stack(
        [data["signal"][v] * s for v, s in VARS_TO_USE.items()]
    )
    ...
    y = jnp.array(
        np.concatenate([y_wjets[:n], y_ttbar[:n]]), dtype=jnp.float32
    )
    ...
    for model_path in model_paths:
        ...
        signal_acc = float(
            accuracy(
                ...
                np.concatenate(
                    [np.ones(len(X_signal)), np.zeros(len(X_test))]
                ),
                prefix=prefix,
            )
        )
        ...
        counts_wjets = ax.hist(
            wjets_score,
            bins=bins,
            label="wjets",
            color=PLOT_COLORS["wjets"],
            alpha=0.5,
            density=True,
        )
        counts_ttbar = ax.hist(
            ttbar_score,
            bins=bins,
            label="ttbar",
            color=PLOT_COLORS["ttbar"],
            alpha=0.5,
            density=True,
        )
        counts_signal = ax.hist(
            signal_score,
            bins=bins,
            label="signal",
        ...
        ax.set_title(
            f"{model_name} | Test acc: {test_acc:.2f} | Signal acc: {signal_acc:.2f}",
            fontsize=12,
            loc="left",
        )
        ...
        save_path = (
            output_dir / "plots" / "scores" / f"model_scores_{model_name}.pdf"
        )
...
def update(params, x, y, lr=0.01, prefix=""):
    loss, grads = jax.value_and_grad(binary_cross_entropy)(
        params, x, y, prefix
    )
        ...
        if epoch % 100 == 0:
            print(
                f"[{epoch}] loss={loss:.4f} \
                    train={history['train_acc'][-1]:.3f} \
                    test={history['test_acc'][-1]:.3f}"
            )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="output_jax_nn_standalone"
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Path(s) to model.pkl to plot scores from.",
    )
    ...
    plot_model_scores(
        [f"{output_dir}/model.pkl"], data, output_dir, prefix=PREFIX
    )
